...
4. 人工核对：委托客户与始发港/目的港可从主数据下拉选取（见「主数据检索」），
   其余字段在原表单上直接修改；
5. 核对完成后由调用方发起真实提交（本服务只产出结构化结果，不代提交）。

## 主数据检索

核对弹窗的委托客户与始发港/目的港支持「输入即下拉」，候选来自 poOrder PublicWebApi
主数据的本地缓存：

- `GET /analysis/customers`：委托客户（按客户 ID / 编码 / 中文名 / 英文名匹配）；
- `GET /analysis/ports`：港口（按三字码 / 英文港口名匹配）。

两者均纯本地匹配、不调用模型；响应中的 `enabled` 为 false 表示未配置对应主数据接口，
此时前端退化为手工填写。客户候选用于回填客户 ID（`fid`），港口候选用于回填三字码
（`sfg` / `mdg`）。
